...
def process_run_events(
    ...
    while run_active:
        try:
            ...
            if "AssistantStreamManager" in str(type(event_stream)):
                print(event_stream.__dir__())
            ...
            with event_stream as es:
                for event in es:
                    ...
                    # Handle the event
                    result, new_event_stream = handle_event(
                        event, run_id, thread_id, front_end_callback
                    )
                    ...
                    if run_id:
                        ...
                        if run_status in ["completed", "failed", "cancelled"]:
                            ...
                            try:
                                ...
                                print(
                                    f"------> Error re-initializing event stream: {e}"
                                )
                                run_active = False
                    else:
                        print("Run ID not obtained; cannot proceed.")
...
def submit_tool_outputs(tool_calls, run_id, thread_id):
    ...
    new_event_stream = client.beta.threads.runs.submit_tool_outputs_stream(
...
def process_tool_function(function_name, arguments):
    ...
    if function_name == "add_numbers":
        print("________________________________ADD NUMBERS____________________________________")
        a = args.get("a", 0)
        b = args.get("b", 0)
        result = a + b
        return json.dumps({"result": result})
    if function_name == "multiply_numbers":
        print("________________________________MULTIPLY NUMBERS____________________________________")
        a = args.get("a", 0)
        b = args.get("b", 0)
        result = a * b
        return json.dumps({"result": result})
